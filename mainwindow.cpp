    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->label_parameterQ->setText("Parameter q");
...
void MainWindow::handlebutton_load(){
    ...
    if (!filepath.isEmpty())
    {
        ...
        ui->label_loaderror->setText("Err: "+QString::number(load_matrix()));
    };
    //else do nothing//
...
int MainWindow::load_matrix(){
    ...
    while (!str.isNull() and (i < n_k) and (j < length_n)) {
        ...
        while((!strStream.atEnd()) and (i < n_k) and (j < length_n)){
            ...
            if (j == length_n)
            {
                ...
                ui->plainTextEdit_matrix->appendPlainText(str2);
                str2 = "";
            }; //endif
    ...
    if (i != n_k)
    {
        ui->label_answer->setText("Matrix is loaded uncorrectly");
        return 1;
    };
